...
Use `generateGuarded()` when a reviewed merge plan should only apply if live
rows still match an expected snapshot.
...
Guarded updates include both identity clauses and expected live-row clauses.

```sql
UPDATE `posts` SET `title` = ? WHERE `id` = ? AND `status` = ?
-- params: ['New Title', 42, 'draft']
```

If the row no longer has the expected `status`, the statement affects zero rows
and guarded apply rolls back the transaction.

### Guarded INSERT

Guarded inserts use `NOT EXISTS` to avoid inserting into an identity that has
appeared since the plan was created.

```sql
INSERT INTO `posts` (`id`, `title`) SELECT ?, ? WHERE NOT EXISTS (
  SELECT 1 FROM `posts` WHERE `id` = ?
)
-- params: [42, 'New Post', 42]
```

## NULL handling

NULL values are passed as PHP `null` in the params array. The connection adapter handles the binding:

```php
$statements = SqlGenerator::generate($result);
// params might contain: [42, null, 'draft']
// The adapter binds null as SQL NULL
```
