...
Merql::init($pdo, identityRules: new IdentityRuleSet([
    'options' => IdentityRule::natural(['option_name']),
]));
```

### `init(PDO $pdo, ?Driver $driver = null, ?IdentityRuleSet $identityRules = null): void`
...
### `__construct(PDO $pdo, ?Driver $driver = null, ?IdentityRuleSet $identityRules = null)`

The driver is auto-detected from the PDO connection if not provided. Pass an
