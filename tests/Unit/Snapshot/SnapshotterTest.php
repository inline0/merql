...
final class SnapshotterTest extends TestCase
{
    ...
    public function capture_uses_explicit_identity_rules(): void
    {
        ...
        $pdo->exec("INSERT INTO options (option_id, option_name, option_value) VALUES (1, 'siteurl', 'https://example.test')");

        $rules = new IdentityRuleSet([
            'options' => IdentityRule::natural(['option_name']),
        ]);

        $snapshot = (new Snapshotter($pdo, identityRules: $rules))->capture('ours', ['options']);
    ...
    {
        $pdo = new PDO('sqlite::memory:');
        $pdo->exec('CREATE TABLE postmeta (meta_id INTEGER PRIMARY KEY, post_id INTEGER, meta_key TEXT, meta_value TEXT)');
        ...
        (new Snapshotter($pdo, identityRules: $rules))->capture('ours', ['postmeta']);
    }
}
